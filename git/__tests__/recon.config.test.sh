...
describe "format-tracking"; (
  ...
  it "replaces [synced] with ✔"; (
    ...
    assert_equal "$expected" "$(remove_colors "$actual")"
  )
)

describe "list-recent-branches"; (
  it "lists recent branches"; (
    expected='Sat 2024-Feb-17 00:12|William Thorsen|bcf331d|git/1||origin/git/1';
    actual=$(devGit list-recent-branches 1 | grep bcf331)
    assert_equal "$expected" "$(remove_colors "$actual")"
  )
)
